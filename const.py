...
class File:
    ...
    def setupAllPaths(self):
        ...
        if self.mainName is None:
            ...
            self.recordFilePathCSV = os.path.join(self.outputFolder, 'summarycsv.csv')
        else:
            # self.mainName -> 'UK'
            self.recordFilePath = os.path.join(self.outputFolder, 'summary_%s.xlsx' % (self.mainName,))
            self.recordFilePathCSV = os.path.join(self.outputFolder, 'summarycsv_%s.csv' % (self.mainName,))
...
class Record:
    ...
    amrClass = 'amrClass'
    recordTitle = 'summary'
    headLabels = ('AssemblyName', 'Subspecies', 'Serovar', 'MLST', 'cgMLST', 'AMR:Gene/Antibiotic',
                  'SPI', 'Spacer-C1', 'Spacer-C2', 'NumSP-C1', 'NumSP-C2','DR-C1', 'DR-C2',
                  'Pos-C1', 'Pos-C2', 'LenC1(bp)', 'LenC2(bp)', )
